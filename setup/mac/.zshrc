...
# Pyenv (must be before Homebrew so shims take priority)
export PYENV_ROOT="$HOME/.pyenv"
[[ -d $PYENV_ROOT/bin ]] && export PATH="$PYENV_ROOT/bin:$PATH"
eval "$(pyenv init - zsh)"

# Don't auto-activate virtualenvs in VS Code's integrated terminal
if [[ "$TERM_PROGRAM" != "vscode" ]]; then
    eval "$(pyenv virtualenv-init -)"
fi

# Add Homebrew completions BEFORE compinit runs
...
zstyle ':fzf-tab:*' switch-group '<' '>'  # optional: switch groups with < >

# Also recommended — disable the standard menu so fzf-tab takes full control
